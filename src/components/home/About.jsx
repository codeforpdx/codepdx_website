...
const renderTopImages = ({ image, label, alt }) => {
  return (
    <Grid item key={alt}>
      <Box
        sx={{
...
const renderBottomImages = ({ image, label, alt }) => {
  return (
    <Grid item key={alt}>
      <Box
        sx={{ display: 'inline-block', borderRadius: '1px', overflow: 'hidden', margin: '20px' }}
      >
        <img
          src={image}
          alt={alt}
...
const About = () => {
  return (
    <Grid container maxWidth={'xl'}>
      ...
      >
        <Typography variant="h3" component="h2" sx={{ px: '1rem', pt: '40px' }}>
          Who is CODE PDX?
        </Typography>
